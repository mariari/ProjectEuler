...
getFactorList x
  | x > 0       = S.toList . S.fromList . filter (\y -> (y < x) && 0 == x `rem` y)
...
-- NEW CODE to 23
amicableList :: [Integer]
amicableList = filter isAmicable [1..28123]

amicablePerms :: [Integer]
amicablePerms = (+) <$> amicableList <*> amicableList


main :: IO ()
main = print . sum $ S.difference (S.fromList [1..28123]) permSet
  where amicSet = S.fromList amicableList
        permSet = S.fromList amicablePerms
