...
-- not memoized
longestPath' :: (Ord a, Num a) => Int -> Int -> V.Vector (V.Vector a) -> (a, [a])
longestPath' x y vec = recurse x y (0, [])
  where recurse x y (acc,path)
          | y == 0      = (acc + loc, loc : path)
          | x == 0      = up
          | x == rowLen = upleft
          | otherwise  = max up upleft 
          where
...
longestPath x y vec = startEvalMemo $ longestPathMem x y vec

longestPathMem :: MonadMemo (Int, Int) (Integer, [Integer]) m => Int -> Int -> V.Vector (V.Vector Integer) -> m (Integer, [Integer])
longestPathMem x y vec = recurse x y
  where
    recurse :: MonadMemo (Int, Int) (Integer, [Integer]) m => Int -> Int -> m (Integer, [Integer])
    recurse x y
      | y == 0      = return (loc,[loc])
      | x == 0      = combine <$> up
      | x == rowLen = combine <$> upleft
      | otherwise  = up >>= \u -> (max (combine u) . combine) <$> upleft
     where
       loc          = vec V.! y V.! x
       rowLen       = length (vec V.! y) - 1
       up           = for2 memo recurse x       (y - 1)
       upleft       = for2 memo recurse (x - 1) (y - 1)
       combine next = (loc + fst next, loc : snd next)

-- the algorithm works by having the (n-1)th row add the best thing from the nth row
-- this continues to happen until we hit the very end
sumTriangle :: (Ord a, Num a) => V.Vector (V.Vector a) -> (a, [a])
sumTriangle vec = V.head $ foldr reduce lastWithPath butLast
  where
    reduce a b          = V.zipWith combine a (V.zipWith max b (V.tail b))
    butLast             = V.reverse . V.tail . V.reverse $ vec
    lastRow             = vec V.! (length vec - 1)

    lastWithPath        = (\x -> (x, [x])) <$> lastRow
    combine a (acc,lis) = (acc + a, a : lis)


-- x = sequence $  (\x -> longestPath'' x len bigvec) <$> [0..len]
answer18 vec = maximum . startEvalMemo . traverse  (\x -> longestPathMem x len vec) $ [0..len]
  where len = (length vec - 1)


main :: IO ()
main = print $ answer18 bigvec

-----------------------------------------------------

bigvec = V.fromList <$> V.fromList [[59]
                                   ,[73, 41]
                                    ,[52, 40, 09]
                                    ,[26, 53, 06, 34]
                                    ,[10, 51, 87, 86, 81]
